...
- **L1. Hand-rolled modals despite Radix being installed. — DONE** `QuickCaptureModal`, `RefileDialog`, `SettingsDialog`, `AgendaBuilderDialog` are plain fixed divs: no focus trap, no Escape-to-close, no scroll lock, no aria roles. `@radix-ui/react-dialog` is already a dependency — wrap each in `Dialog.Root/Portal/Content`. (Also `bg-opacity-50` is deprecated syntax in Tailwind v4 — use `bg-black/50`.)
  **Done** — New shared `src/components/ui/Dialog.tsx` wraps `Dialog.Root/Portal/Overlay/Content` (with a required `Dialog.Title` for a11y, a `center`/`top` position variant to preserve `RefileDialog`'s near-top placement, and `bg-black/50` instead of the deprecated `bg-opacity-50`). All four modals now render through it instead of a raw fixed `<div>`, picking up focus trap, Escape-to-close, scroll lock, and aria roles for free from Radix. Couldn't visually verify in a browser in this headless environment (no display/Tauri window available) — verified via `tsc -b`/`vite build` only.
- **L2. Watcher test is slow and timing-flaky. — DONE** (`file_watcher.rs:80-106`, two 2-second sleeps). Poll with a timeout loop instead of fixed sleeps, or feature-gate it as an integration test.
  **Done** — Already resolved as a side effect of the H5/H6 rewrite: every watcher test now uses a `wait_until` polling helper (50ms interval, 5s timeout) instead of fixed sleeps, except one deliberate `thread::sleep(1500ms)` in `test_remove_watched_folder_stops_watching_and_drops_tasks` to confirm an *absence* of a change past the debounce window — which can't be replaced by polling since there's no positive condition to wait for. No code change needed.
- **L3. `parse_org_file`/`parse_org_content` commands construct a fresh `OrgParser`. — DONE** instead of using `State<AppState>` — harmless now (the parser is stateless), but once ParseConfig comes from settings (M2) they'll silently use the wrong keywords. Route them through `AppState` when doing C6.
  **Done** — Both commands now take `state: State<AppState>` and call `state.parser.parse_content`/`parse_file` instead of constructing `OrgParser::new()`.
- **L4. `list_tasks` filter is case-sensitive substring. — DONE** (`commands.rs:142-148`) while the frontend filter (`TaskListPane.tsx:79-93`) is case-insensitive and does its own filtering anyway — decide where filtering lives (frontend-only is fine at this scale) and delete the other.
  **Done** — Filtering lives frontend-only: `list_tasks` dropped its `filter: Option<String>` parameter entirely and now just returns every task via `TaskStore::get_all_tasks`. `TaskStore::filter_tasks` (the generic predicate method) stays, since `get_agenda_range` still uses it.
- **L5. `paneSizes` state loop. — DONE**: `MainLayout` feeds `paneSizes` into `defaultSize` and writes every resize back to the store, but `defaultSize` is only read on mount — the store write is currently useless. Either persist pane sizes (zustand `persist` middleware) so they survive restarts, or drop `setPaneSizes`.
  **Done** — `uiSlice.ts`'s `useUiSlice` is now wrapped in zustand's `persist` middleware with `partialize` limited to `paneSizes` (not `activeModal`/`isVimMode`), so a resize survives an app restart instead of always resetting to `[30, 70]`.
- **L6. Pinned alpha dependency. — DONE**: `orgize = "0.10.0-alpha.10"` is a pre-release; API churn is expected. Fine for now, but pin exactly (`=0.10.0-alpha.10`) so a `cargo update` can't break the parser silently.
  **Done** — `Cargo.toml`'s `orgize` dependency changed to `"=0.10.0-alpha.10"`.
- **L7. No JS test runner. — DONE** despite `build-plan/tasks/tasks-frontend.md` referencing `pnpm exec jest`. Add Vitest (`pnpm add -D vitest @testing-library/react jsdom`) — it shares the Vite config, and the recursive tree helpers in `tasksSlice.ts`/`TaskListPane.tsx` are perfect first test targets.
  **Done** — Added `vitest`, `@testing-library/react`, `@testing-library/jest-dom`, `@testing-library/user-event`, and `jsdom`; `vite.config.ts` gained a `test: { environment: 'jsdom', setupFiles: [...] }` block (shares the Vite/React config as suggested) and `package.json` gained `test`/`test:watch` scripts. While extracting these helpers for testing, `buildTaskTree`/`findTask`/`mapTaskRecursively`/`removeTaskRecursively` (from `tasksSlice.ts`) and `filterTasks` (from `TaskListPane.tsx`) were consolidated into a new `src/lib/taskTree.ts` — they were pure `Task[]` tree operations duplicated/scattered across a store and a component, and exporting a plain function alongside a component from the same file trips `react-refresh/only-export-components`. `tasksSlice.ts` now imports from and re-exports the `Task`/`TodoState` types from there so every existing `import type { Task } from '../stores/...'` kept working unchanged. New tests: `src/lib/taskTree.test.ts` (13 cases covering all 5 helpers) and `src/components/TaskListPane.test.tsx` (a render/interaction smoke test typing into the filter input). All 14 pass via `pnpm test`.
- **L8. `pnpm` blocks builds in fresh checkouts. — DONE**: `esbuild`/`@tailwindcss/oxide` build scripts are unapproved (`pnpm approve-builds`). Commit the approval (pnpm stores it in `package.json` > `pnpm.onlyBuiltDependencies`) so `pnpm install && pnpm build` works from a clean clone.
  **Done** — Found an untracked, broken `pnpm-workspace.yaml` already sitting in the repo (`allowBuilds: { esbuild: "set this to true or false", ... }` — placeholder text, not valid pnpm config) that was silently doing nothing. Replaced it with pnpm 11's actual schema: `onlyBuiltDependencies: [esbuild, '@tailwindcss/oxide']` (this pnpm version stores the approval in `pnpm-workspace.yaml`, not `package.json`'s `pnpm` field). Verified via `pnpm config list` that both packages are now recognized.
- **L9. Task checklists are marked complete but the code isn't. — DONE** Both `build-plan/tasks/*.md` files show everything `[x]`, yet C4–H6 above are unfinished. Per your own `.cursor/rules/process-tasks.mdc` workflow, un-check the items that aren't actually done (esp. backend 3.x/4.x command persistence and agenda) so the plan reflects reality.
  **Done** — By now (after all of C1–M10 above), backend command persistence and agenda filtering are genuinely implemented, so no unchecking was needed there — the finding predates those fixes. What *is* now stale, annotated in place rather than blindly unchecked (since these items were genuinely done, then later legitimately changed): `tasks-frontend.md` 5.5 (`UpdatePrompt.tsx` — later deleted, M7), 11.2 (`RefileDialog` — mock targets replaced with real ones, M7), 12.4 (settings sync — actual command names differ from the placeholder `getSettings`/`saveSettings`), its Notes section (`pnpm exec jest` → `pnpm test`, now that Vitest exists per L7), and its Relevant Files list (added `lib/api.ts`, `lib/taskTree.ts`, `ui/Dialog.tsx`, `ui/Dropdown.tsx`, the two dialogs it hadn't listed, `pnpm-workspace.yaml`; removed the deleted `tailwind.config.js`). `tasks-backend.md` 5.4 similarly annotated for L4's `list_tasks` filter removal.
